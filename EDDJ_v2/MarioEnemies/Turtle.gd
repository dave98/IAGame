...
func state_2():
	...
	velocity.x = 0
	$Sprite.play("koopa_shield_idle" + self.process_turtle_type)
	velocity.y += GRAVITY
	velocity = move_and_slide(velocity, FLOOR)
...
func state_1():
	...
	if turtle_type != 2: # La tortuga verdad caera siempre de los barrancos
...
func set_shield_direction(var b_x, var b_self):
	if b_x < b_self:
		self.direction = 1
	else:
		self.direction = -1
...
func _on_TouchBody_body_entered(body):
	if "Player" in body.name:
		...
		if self.is_above(body.position.y, self.position.y):
			...
			if self.states == 3:
				...
				self.increase_state()
				self.set_shield_direction(body.position.x, self.position.x)	
		else:
			if self.states == 2:
				self.set_shield_direction(body.position.x, self.position.x)	
				self.states = 3
	...
	elif "Turtle" in body.name and self != body:
		if self.states == 3:
			$Sounds/stomp.play()
			body.kill_character() 
	
	elif "Soccer" in body.name:
		if self.states == 3:
			$Sounds/stomp.play()
			body.kill_character() 
